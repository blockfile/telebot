...
| --- | --- | --- |
| `watch.triggerMarketCapUsd` | The market cap (in USD) a token must reach during the 90-minute watch window before it's promoted to the deep-check stage. Default `5000`. | Lower = catches tokens earlier (more alerts, more noise). Higher = only well-established tokens get checked (fewer, later alerts). |
| `watch.triggerVolumeUsd` | The total traded volume (in USD, buys + sells, accumulated while the token is watched) it must reach before promotion. This is the "is it actually being traded?" check — it stops dead tokens that merely drifted up to the market-cap line. Default `10000`. | Higher = demands more real trading activity (fewer alerts, higher conviction). Lower = lets quieter tokens through sooner. |
| `watch.triggerUniqueBuyers` | The number of distinct wallets that must have bought the token during the watch window before it's promoted to the deep-check stage. Default `15`. This is your anti-wash-trade guard — it stops a token where a few bundled wallets faked all the volume. | Lower = earlier but noisier alerts, more wash-trade risk. Higher = requires a broader real crowd before checking further. |
